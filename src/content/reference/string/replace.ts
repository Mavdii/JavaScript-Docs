...
export const stringReplaceReference: ReferenceContent = {
  ...
  sections: [
    ...
    { type: 'code', language: 'javascript', filename: 'replace-mistakes.js', code: `// Wrong: strings only replace the first match\nconst text = 'apple apple apple';\ntext.replace('apple', 'orange'); // 'orange apple apple'\n// Correct: use replaceAll() for all matches\ntext.replaceAll('apple', 'orange'); // 'orange orange orange'\n\n// Wrong: forgetting the /g flag with regex\ntext.replace(/apple/, 'orange'); // 'orange apple apple'\n// Correct: use /g for all matches\ntext.replace(/apple/g, 'orange'); // 'orange orange orange'\n\n// Wrong: using an unescaped $ in a regex means "end of string"\nconst price = '$10';\nprice.replace(/$/, 'EUR'); // '$10EUR'\n// Correct: escape $ in the regex\nprice.replace(/\\$/, 'EUR'); // 'EUR10'\n\n// Wrong: expecting the original string to be modified\nconst str = 'hello';\nstr.replace('hello', 'hi'); // Returns a new string\nconsole.log(str); // 'hello' (unchanged)\n// Correct: capture the result\nconst newStr = str.replace('hello', 'hi');` },
