...
export const stringTrimReference: ReferenceContent = {
  ...
  sections: [
    ...
    { type: 'code', language: 'javascript', filename: 'trim-mistakes.js', code: `// Wrong: assuming trim removes interior whitespace\nconst text = 'hello   world';\ntext.trim(); // 'hello   world' (interior spaces kept)\n// Correct: use replace to collapse interior spaces\ntext.replace(/\\s+/g, ' '); // 'hello world'\n\n// Wrong: forgetting trim doesn't mutate\nconst userInput = '  text  ';\nuserInput.trim(); // Returns a new string\nconsole.log(userInput); // '  text  ' (unchanged)\n// Correct: assign the result\nconst cleaned = userInput.trim();\n\n// Wrong: not trimming before validation\nfunction validate(input) {\n  return input.length > 0; // Returns true for '   '\n}\n// Correct: trim first\nfunction validate(input) {\n  return input.trim().length > 0;\n}\n\n// Wrong: trimming an array directly\nconst items = ['  a  ', '  b  ', '  c  '];\nitems.trim(); // TypeError: items.trim is not a function\n// Correct: map to trim each item\nconst trimmed = items.map(item => item.trim());` },
